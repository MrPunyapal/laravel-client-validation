...
export class VanillaFormValidator {
  constructor(form, options = {}) {
    ...
    this.validator = null;
    this.eventListeners = [];
    this.form._vanillaFormValidator = this;
    this.init();
  }
  ...
  destroy() {
    ...
    if (this.form._vanillaFormValidator === this) {
      delete this.form._vanillaFormValidator;
    }
  }
}
...
export function initForms(selector = 'form[data-validate]') {
  ...
  const validators = [];
  forms.forEach(form => {
    // Skip forms that already have a validator so repeated initForms/autoInit
    // calls never attach duplicate listeners.
    if (form._vanillaFormValidator) return;
